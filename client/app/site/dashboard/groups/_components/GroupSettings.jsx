    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select a group to view and edit its settings</CardDescription>
        </CardHeader>
      </Card>
    )
  }
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Enable or disable rules for your group</p>
          <div className="mt-4 space-y-4">
            {rules.map(rule => (
              <div key={rule.id} className="flex items-center justify-between">
                <Label htmlFor={rule.id} className="flex items-center cursor-pointer">
                  {rule.label}
                </Label>
                <Switch
                  id={rule.id}
